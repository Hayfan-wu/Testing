...
class BaiduNetdisk:
    ...
    def coins_tasks(self):
        ...
        video_task_info = None  # 视频任务信息(需App端完成)
        ...
        for t in tasks:
            ...
            if t.get("is_today_finish") == 1:
                today_done += 1
            # 视频任务(type=70)特殊处理: 需在App端看广告，脚本无法完成
            if ttype == "70":
                finish_count = t.get("task_finish_count", 0)
                count_max = t.get("task_count_max", 15)
        ...
        # 第一步: 对未开始的非SVIP、非视频任务调用 tasksave 上报
        saved = 0
        saved_score = 0
        failed = 0
        for t in tasks:
            status = t.get("task_status", 0)
            ttype = t.get("task_type", "")
            # 跳过: SVIP任务(114)、视频任务(70,需App端看广告)
            if status != 0 or ttype in ("114", "70"):
                continue
