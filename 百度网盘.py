...
BAIDU_BOX_ROUNDS (可选) 宝箱领取轮数，默认 3，范围 0-10。
                  每轮需驱动倒计时(约1-4分钟)，设 0 跳过宝箱领取。
...
class BaiduNetdisk:
    ...
    def _antisave(self, task):
        """尝试领取任务奖励(antisave 接口)。

        使用 clienttype=0(Web) 绕过 HTJ 反作弊检查。
        返回 (errno, addScore, msg) 元组。
        注意: clienttype=0 返回 errno=0 但 taskStatus 仍为6(已上报)，
        部分任务可能通过此方式成功领取积分，但大部分需要App端完成。
        """
        task_id = task.get("task_id_str", "")
        task_from = task.get("task_from", "")
        if not task_id or not self.uk:
        ...
        params["clienttype"] = "0"  # 关键: Web客户端绕过HTJ
    ...
    def coins_tasks(self):
        ...
        流程: tasksave(标记已上报 status 0->6) -> antisave(尝试领取积分)。
        antisave 受HTJ反作弊保护，脚本使用 clienttype=0 尽力尝试，
        部分任务可成功领取，其余需在App端手动完成。
        """
        ...
        if tdata.get("errno") != 0:
            self.results.append(("任务列表查询", "失败", tdata.get("error", "未知错误")))
            return
        ...
        # 统计任务状态
        done = 0       # status==1 (历史已完成)
        reported = 0   # status==6 (已上报但未领取奖励)
        not_started = 0  # status==0
        svip_skip = 0  # SVIP任务跳过
        today_done = 0  # 今日已完成(is_today_finish=1)
        total_score_done = 0  # 已完成任务总积分
        total_score_reported = 0  # 已上报任务总积分(待领取)

        for t in tasks:
            status = t.get("task_status", 0)
            ttype = t.get("task_type", "")
            score = t.get("task_score", 0) or 0
            if t.get("is_today_finish") == 1:
                today_done += 1
            if status == 1:
                done += 1
        ...
        self.results.append(("任务列表查询", "信息",
                             "共%s个: 已完成%s(+%s分) 已上报%s(待领+%s分) 未开始%s SVIP跳过%s" %
                             (len(tasks), done, total_score_done, reported, total_score_reported, not_started, svip_skip)))

        # 第一步: 对未开始的非SVIP任务调用 tasksave 上报
        saved = 0
        saved_score = 0
        failed = 0
        for t in tasks:
        ...
        if saved > 0:
            self.results.append(("任务上报", "成功", "上报 %s 个任务(约+%s积分待领取)" % (saved, saved_score)))
        if failed > 0:
            self.results.append(("任务上报", "失败", "%s 个任务上报失败" % failed))

        # 第二步: 对已上报(status=6)的任务尝试 antisave 领取积分
        claimed = 0
        total_score = 0
        already = 0
        htj_blocked = 0
        for t in tasks:
            ...
            if status != 6:
                continue
            errno, add_score, msg = self._antisave(t)
            if errno == 0 and add_score > 0:
                claimed += 1
                total_score += add_score
            elif errno == 40003 or errno == 40004:
                already += 1  # 已领取过
            elif errno in (8001, 8002):
                htj_blocked += 1  # HTJ反作弊拦截
            time.sleep(0.5)
    ...
    # ---------- 主流程 ----------
    def run(self, box_rounds=3, task_save=True):
...
def main():
    ...
    # 宝箱领取轮数(每轮需驱动倒计时，耗时较长; 默认3，最大10; 设0跳过宝箱)
    box_rounds = int(os.getenv("BAIDU_BOX_ROUNDS", "3") or "3")
