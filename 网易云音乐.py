...
def aes_decrypt(hex_data: str) -> str:
    """AES-128-CBC 解密 hex 字符串"""
    cipher = AES.new(EAPI_KEY, AES.MODE_CBC, EAPI_IV)
    pt = cipher.decrypt(binascii.unhexlify(hex_data))
    return unpad(pt, AES.block_size).decode('utf-8', errors='replace')


def eapi_encrypt(url_path: str, params: dict) -> str:
    """
    EAPI 请求体加密 (标准网易云 EAPI 格式)
    格式: nobody{url}use{json_params}end -> MD5 -> {url}-{md5}-{json_params} -> AES-128-CBC
    url_path 应为 /api/ 开头的完整路径
    """
    text = json.dumps(params, separators=(',', ':'), ensure_ascii=False) if isinstance(params, dict) else str(params)
    message = f"nobody{url_path}use{text}end"
    digest = hashlib.md5(message.encode()).hexdigest()
    data_to_encrypt = f"{url_path}-{digest}-{text}"
    return aes_encrypt(data_to_encrypt)
...
class NeteaseCloudMusic:
    ...
    def __init__(self, music_u: str):
        ...
        self.s.headers.update({
            "User-Agent": "NeteaseMusic/9.4.90.240411134925(9.4.90);Dalvik/2.1.0(Linux;U;Android 13;PFEM00 Build/TP1A.220905.001)",
            ...
            "X-Real-IP": "116.252.0.1",
            "Referer": "https://music.163.com",
        })
        self.s.cookies.set("MUSIC_U", music_u, domain=".music.163.com")
        self.s.cookies.set("os", "android", domain=".music.163.com")
        self.s.cookies.set("appver", "9.4.90", domain=".music.163.com")
        self.s.cookies.set("versioncode", "240", domain=".music.163.com")
        self.nickname = ""

    def _eapi(self, endpoint: str, params: dict = None) -> dict:
        """EAPI 加密请求 (逆向自 k42.e.b() -> j52.a)
        加密时用 /api/ 路径, 请求时发往 /eapi/ 路径
        """
        api_path = f"/api/{endpoint}"
        eapi_url = f"{BASE_URL}/eapi/{endpoint}"
        p = params or {}
        p.setdefault("header", {"os":"android","appver":"9.4.90","versioncode":"240","build":"240411134925"})
        enc = eapi_encrypt(api_path, p)
        try:
            r = self.s.post(eapi_url, data={"params": enc}, timeout=15)
            text = r.text.strip()
            if not text:
                print(f"  [!] EAPI {endpoint}: 空响应 (HTTP {r.status_code})")
                return {}
            # EAPI 响应通常是加密的 hex 字符串
            try:
                ...
                try:
                    ...
                    print(f"  [!] EAPI {endpoint}: 响应解析失败 (HTTP {r.status_code}), 前100字符: {text[:100]}")
                    return {}
        except requests.exceptions.ConnectionError:
            print(f"  [!] EAPI {endpoint}: 网络连接失败")
            return {}
    ...
    def _api(self, endpoint: str, params: dict = None, method="POST") -> dict:
        ...
        try:
            ...
            text = r.text.strip()
            if not text:
                print(f"  [!] API {endpoint}: 空响应 (HTTP {r.status_code})")
                return {}
            return json.loads(text)
        except Exception as e:
    ...
    def daily_signin(self) -> dict:
        """每日签到 (优先标准 API, 回退 EAPI)"""
        # 标准签到 API
        result = self._api("point/dailySignIn", {"type": "0"})
    ...
    def get_user_info(self) -> dict:
        """获取用户信息 (标准 API, 不需要 EAPI 加密)"""
        # 优先用 /api/w/nuser/account/get (更稳定)
        result = self._api("w/nuser/account/get", method="GET")
        if result.get("code") == 200 or result.get("account"):
            return result
        # 回退到 /api/v1/user/info
        return self._api("v1/user/info", method="GET")

    def get_personal_fm(self) -> dict:
        """获取私人FM推荐歌曲 (优先标准 API)"""
        result = self._api("v1/radio/get", method="GET")
    ...
    def get_recent_songs(self) -> dict:
        """获取最近播放歌曲 (优先标准 API)"""
        result = self._api("v1/play/record", {"limit": "10", "offset": "0"}, method="GET")
        if result.get("code") == 200:
            return result
    ...
    def run(self) -> str:
        ...
        if info.get("code") == 200 or info.get("account"):
            # w/nuser/account/get 返回 account + profile
            p = info.get("profile", {})
            if not p:
                p = info.get("account", {})
            if p:
        ...
        if fm.get("code") == 200:
            ...
            if isinstance(d, list):
                ...
                songs = d.get("data", d.get("list", []))
        if songs:
            for song in songs[:5]:
                # v1/radio/get 直接返回歌曲对象; play/record 返回 {song: {...}}
                actual_song = song.get("song", song) if isinstance(song, dict) else {}
                sid = actual_song.get("id", song.get("id", song.get("songId", "")))
                if not sid: continue
                sname = actual_song.get("name", song.get("name", f"ID:{sid}"))
                # 获取统计
                st = self.get_song_stats(sid)
                rc = st.get("data", {}).get("rcmdCount", 0)
                # 领取
                ruid = song.get("rcmdUserId", actual_song.get("rcmdUserId", 0))
        ...
        if recent.get("code") == 200:
            ...
            if isinstance(d, list):
                ...
                r_songs = d.get("list", d.get("data", d.get("allData", [])))
        if r_songs:
            for song in r_songs[:5]:
                actual_song = song.get("song", song) if isinstance(song, dict) else {}
                sid = str(actual_song.get("id", song.get("id", song.get("songId", ""))))
