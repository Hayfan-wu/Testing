...
"""
名称: 百度网盘每日任务
功能: 自动完成百度网盘会员成长值签到、积分任务中心签到、宝箱领取及状态查询
作者: TRAE
日期: 2026-07-30
...
============================== 已验证接口情况 ==============================
【确认可用 - 无需 z 签名，仅需 Cookie+设备参数(cuid/devuid 可用任意值)】
  - GET /rest/2.0/membership/level?method=signin   会员成长值签到(每日)
  ...
  - GET /coins/center/boxaward                     领取宝箱(需 ptype=5，已实测+5积分)

【尽力而为 - 需用户核实】
  - GET /coins/taskcenter/signin                   积分任务中心签到
    (抓包仅有读取请求；写入参数未能完全确定，今日已签时返回 param error。
     会员成长值签到可能已自动完成该签到任务，待未签到日核实。)
  如需精确写入参数，请在手机端实际点击「签到」时抓包补全。
...
class BaiduNetdisk:
    ...
    def coins_signin(self):
        ...
        if sdata.get("errno") == 0:
            ...
            if signed:
                self.results.append(("积分任务中心签到", "已签", "今日已签到，无需重复"))
                return
        else:
            self.results.append(("签到状态查询", "失败", sdata.get("error", "未知错误")))
        # 尝试签到(写入参数未能完全确定，尽力尝试; 已带 ptype=5)
        rdata = self._coins_get("/coins/taskcenter/signin", {"task_from": "task_sys_daily", "ptype": "5"})
        code = rdata.get("errno", -1)
        err = rdata.get("error", "")
        if code == 0:
            self.results.append(("积分任务中心签到", "成功", "签到成功"))
        elif code == 2:
            self.results.append(("积分任务中心签到", "待核实", "返回 param error(可能今日已签/缺少写入参数，需抓包补全)"))
        else:
    ...
    def coins_status(self):
        ...
        if tdata.get("errno") == 0:
            ...
            done = sum(1 for t in tasks if t.get("task_status", 0) >= 1 or t.get("task_finish_count", 0) >= t.get("task_count_max", 1))
            self.results.append(("任务列表查询", "信息", "共 %s 个任务，已完成 %s" % (len(tasks), done)))
    ...
    def coins_box(self, max_rounds=3):
        ...
        for attempt in range(max_rounds):
            ...
            if status == 1:
                ...
                time.sleep(1)
                continue
            # 未就绪，驱动倒计时至归零(每次 tick 约推进 1 秒真实时间)
            self.results.append(("宝箱倒计时", "进行中", "第 %s/%s 轮剩余 %s 秒，开始驱动倒计时" % (rnd, total, remain)))
            tick_count = 0
            max_ticks = remain + 30  # 基于初始 remain 固定上限(避免随 remain 减小而提前退出)
...
def main():
    ...
    for idx, ck in enumerate(cookies):
        ...
        results = bd.run(box_rounds=box_rounds)
        lines = []
