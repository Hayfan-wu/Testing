...
格式（多账号用换行 \n 分隔）:
  账号#密码#支付宝姓名#支付宝账号#设备id

示例:
  13800138000#mypassword#张三#zhangsan@qq.com#a1b2c3d4e5f6

字段说明:
  账号       - 甬派App登录手机号
  密码       - 甬派App登录密码
  支付宝姓名 - 提现绑定的支付宝真实姓名
  支付宝账号 - 提现绑定的支付宝账号（手机号或邮箱）
...
WxPusher通知配置:
  ...
  2. 环境变量 WX_PUSHER_UID: 接收消息的用户UID（在WxPusher公众号获取，多个用逗号分隔）
  如不需要通知，可不配置以上变量，脚本会自动跳过推送
...
const WX_PUSHER_UID = process.env.WX_PUSHER_UID || ''              // 接收消息的用户UID，多个用逗号分隔

async function sendWxPusher(title, content) {
    if (!WX_PUSHER_APP_TOKEN || !WX_PUSHER_UID) {
        console.log('未配置WxPusher环境变量，跳过通知推送');
        return false;
    }
    try {
        const body = {
            ...
            content: `${title}\n\n${content}`,
            contentType: 1, // 1=文本
            uids: WX_PUSHER_UID.split(',').map(s => s.trim()).filter(Boolean)
        };
        ...
        if (data.code === 1000) {
            ...
            console.log(`WxPusher推送失败: ${data.msg || resp.body}`);
        }
    } catch (e) {
        console.log(`WxPusher推送异常: ${e.message}`);
    }
}
...
function Env(name) {
    return new class {
        ...
        async showmsg() {
            ...
            this.log('\n============== 推送 ==============')
            await sendWxPusher(this.name, this.notifyStr.join('\n'))
        }
