 ...
 *
 * ====================== 关于 jwtToken 有效期 ======================
 *  jwtToken 的 exp 字段为 2 小时，但实测服务器不强制校验过期时间，
 *  只要 Cookie 中的 cookieTokenKey（长期有效）和 cookieToken 没失效即可。
 *  脚本会自动尝试刷新 ssoToken，无需手动干预。
 */
...
// axios 实例：强制 IPv4 + 跳过证书校验（解决 Docker 环境网络问题）
const httpClient = axios.create({
    timeout: 30000,
    // 强制使用 IPv4，避免 Docker 中 IPv6 不可用导致 DNS 解析失败
    family: 4,
    // 跳过 TLS 证书校验（部分青龙环境缺少 CA 证书）
...
async function apiGet(account, path, queryParams = {}) {
    ...
    try {
        const resp = await httpClient.get(url, { headers, params: queryParams });
        return resp.data;
    } catch (e) {
        if (e.response) {
        ...
        $.logErr(`[GET] ${path} 异常: ${e.message} (code: ${e.code || "N/A"})`);
        return null;
    }
}
...
async function apiPost(account, path, body = null, queryParams = {}) {
    ...
    const headers = buildHeaders(account);
    try {
        const resp = await httpClient.post(url, body || undefined, { headers, params: queryParams });
        return resp.data;
    } catch (e) {
        if (e.response) {
        ...
        $.logErr(`[POST] ${path} 异常: ${e.message} (code: ${e.code || "N/A"})`);
        return null;
    }
}
...
/**
 * 刷新 ssoToken（cookieToken）
 * 调用 querySpecToken 接口，用现有 Cookie 换取新的 ssoToken
 * 服务器不强制校验 jwtToken 的 exp 过期时间，只要 cookieTokenKey 有效即可
 */
async function refreshSsoToken(account) {
    ...
    if (result && result.code === 0 && result.result) {
        ...
        $.log(`✅ ssoToken 刷新成功: ${newToken.slice(0, 20)}...`);

        // 更新 Cookie 中的 cookieToken
        if (account.cookie.includes("cookieToken=")) {
            account.cookie = account.cookie.replace(
                /cookieToken=[^;]*/,
...
async function main() {
    ...
    cookieList.forEach((c, i) => {
        $.log(`   [#${i + 1}] 长度=${c.length} 开头: ${c.slice(0, 50)}...`);
    });
    $.log("");
    ...
    for (const cookieStr of cookieList) {
        ...
        if (!acc.jwtToken && !acc.cookieTokenKey) {
            $.log(`⚠️ 跳过无效 Cookie (缺少 jwtToken 和 cookieTokenKey)`);
            continue;
        }
        accounts.push(acc);
