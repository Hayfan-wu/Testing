...
async function httpRequest(method, account, path, body, queryParams) {
    ...
    try {
        ...
        // 如果是 DNS 或网络级错误，尝试不加自定义 Agent 重试一次
        if (e.code === "ERR_INVALID_IP_ADDRESS" || e.code === "ENOTFOUND" || e.code === "EAI_AGAIN") {
            try {
                $.log(`  ⚠️ 尝试直连重试...`);
                ...
                if (typeof data2 === "string") {
                    try { data2 = JSON.parse(data2); } catch (e2) {}
                }
                return { success: resp2.status >= 200 && resp2.status < 300, data: data2, status: resp2.status };
            } catch (e2) {
                $.logErr(`  重试也失败: ${e2.message}`);
            }
        }
