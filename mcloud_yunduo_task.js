 ...
 *
 *  ┌─ 环境变量 (青龙 → 环境变量) ───────────────────────────────────────┐
 *  │ MCLOUD_TOKEN        必填。支持三种格式 (自动识别):                │
 *  │                     ① Authorization值#手机号  (推荐, 最稳定)      │
 *  │                     ② ssoToken  (YZsidssolg开头)                  │
 *  │                     ③ jwtToken  (eyJ开头, 2小时有效期)            │
 *  │ MCLOUD_TOKEN_TYPE   可选。auth / sso / jwt  (不填则自动识别)       │
 *  │ MCLOUD_MARKET       活动名称, 默认 National_Activity               │
 *  │ MCLOUD_PHONE        手机号(auth模式含在TOKEN中, sso/jwt模式可选)   │
 ...
 *  │ 配合手机号用 # 分隔, 可含或不含 "Basic " 前缀                     │
 *  │ Authorization 有效期较长(数月), 可本地解码检查过期时间            │
 *  │ 认证链: Authorization → ssoToken → jwtToken → 执行任务            │
 *  └────────────────────────────────────────────────────────────────────┘
 *
 *  多账号:  用换行或 & 分隔多个 token
 *
...
const ORCHES_HOST = 'orches.yun.139.com';          // Authorization → ssoToken 换发
const CAIYUN_HOST = 'caiyun.feixin.10086.cn';      // ssoToken → jwtToken 换发 (旧版接口)
const CAIYUN_PORT = 7071;
...
const TOKEN_TYPE = (ENV.MCLOUD_TOKEN_TYPE || ENV.mcloud_token_type || 'auto').toLowerCase();
...
// ==================== 通用跨域请求 (orches / caiyun) ====================

/**
 * 向任意主机发送 HTTPS 请求 (用于 Authorization 换发链)
 * @param {string} method  GET / POST
 * @param {string} host    目标主机
 * @param {number} port    端口 (默认 443)
 * @param {string} fullPath  完整路径 (含 query string)
 * @param {object} opts    { body, headers }
 */
function requestTo(method, host, port, fullPath, opts = {}) {
    return new Promise((resolve, reject) => {
        const headers = {
            'User-Agent': UA,
            'Accept': '*/*',
...
// ==================== Authorization 模式: 解析与换发 ====================

/**
 * 自动识别 token 类型
 * @returns {object} { type, token, phone, authorization }
 */
function detectTokenType(rawToken) {
    const trimmed = rawToken.trim();

    // auth 模式: 含 # 分隔符 → Authorization值#手机号
    if (trimmed.includes('#')) {
        ...
        if (auth && phone) {
            ...
            return { type: 'auth', token: auth, phone, authorization: auth };
        }
    }

    // jwt 模式: 以 eyJ 开头 (JWT base64)
    if (trimmed.startsWith('eyJ')) {
        return { type: 'jwt', token: trimmed, phone: '', authorization: '' };
    }

    // sso 模式: 以 YZsidssolg 开头
    if (trimmed.startsWith('YZsidssolg')) {
        return { type: 'sso', token: trimmed, phone: '', authorization: '' };
    }

    // 含 Basic 前缀但无手机号 → 尝试当 auth (无手机号可能换发失败)
    if (trimmed.toLowerCase().startsWith('basic ')) {
        return { type: 'auth', token: trimmed, phone: '', authorization: trimmed };
    }
...
/**
 * 检查 Authorization 有效期 (本地解码 Base64)
 * Authorization 格式: Basic base64(field0:field1:field2|field3=timestamp|...)
 * @returns {boolean} true=未过期, false=已过期
 */
function checkAuthExpiry(authorization) {
    try {
        const raw = authorization.replace('Basic ', '');
        ...
        if (parts.length > 3) {
            ...
            if (expireAt > 0) {
                const now = Date.now();
                if (now > expireAt) {
                    log(`Authorization 已过期! 过期时间: ${new Date(expireAt).toLocaleString('zh-CN')}`);
                    return false;
                }
                const daysLeft = Math.floor((expireAt - now) / 86400000);
                log(`Authorization 剩余 ${daysLeft} 天`);
                if (daysLeft <= 3) log(`Authorization 即将过期, 请及时更新!`);
                return true;
            }
        }
    } catch (e) {
        log(`无法解析 Authorization 有效期: ${e.message}`);
    }
    return true;  // 无法解析时不阻止
}

/**
 * 从 jwtToken 中提取 userDomainId
 */
 ...
 * POST https://orches.yun.139.com/orchestration/auth-rebuild/token/v1.0/querySpecToken
 * @returns {string|null} ssoToken
 */
async function getSsoTokenByAuth(authorization, phone) {
    try {
        const res = await requestTo('POST', ORCHES_HOST, 443,
            '/orchestration/auth-rebuild/token/v1.0/querySpecToken', {
                headers: {
                    'Authorization': authorization,
                    'Content-Type': 'application/json',
                },
                body: { account: phone, toSourceId: SOURCE_ID },
            });
        if (res.data && res.data.success && res.data.data && res.data.data.token) {
            return res.data.data.token;
        }
        log(`ssoToken 换发失败: ${JSON.stringify(res.data).slice(0, 200)}`);
        return null;
    } catch (e) {
        log(`ssoToken 换发异常: ${e.message}`);
        return null;
    }
}

/**
 * ssoToken → jwtToken (通过 caiyun 旧版接口)
 * POST https://caiyun.feixin.10086.cn:7071/portal/auth/tyrzLogin.action?ssoToken=XXX
 * @returns {string|null} jwtToken
 */
async function getJwtByCaiyun(ssoToken) {
    try {
        const res = await requestTo('POST', CAIYUN_HOST, CAIYUN_PORT,
            `/portal/auth/tyrzLogin.action?ssoToken=${encodeURIComponent(ssoToken)}`, {
                headers: {
                    'Host': `${CAIYUN_HOST}:${CAIYUN_PORT}`,
                },
            });
        if (res.data && res.data.code === 0 && res.data.result && res.data.result.token) {
            return res.data.result.token;
        }
        log(`caiyun jwtToken 换发失败: ${JSON.stringify(res.data).slice(0, 200)}`);
        return null;
    } catch (e) {
        log(`caiyun jwtToken 换发异常: ${e.message}`);
        return null;
    }
}
...
class McloudAccount {
    constructor(token, type, marketName, authorization = '', phone = '') {
        this.token = token.trim();
        this.type = type;                          // auth / sso / jwt
        this.marketName = marketName;
        this.authorization = authorization;          // auth 模式: Basic xxx
        this.phone = phone || PHONE;                 // auth 模式从 TOKEN 解析, 否则用环境变量
        this.ssoToken = '';                          // auth 模式换发得到的 ssoToken
        this.jwtToken = '';
    ...
    /** Authorization 模式: auth → ssoToken → jwtToken (三级换发) */
    async loginByAuth() {
        log('使用 Authorization 模式');

        // 1. 本地检查 Authorization 有效期
        if (!checkAuthExpiry(this.authorization)) {
            log('Authorization 已过期, 请重新抓取!');
            return false;
        }

        // 2. 尝试命中 jwtToken 缓存 (以 authorization 哈希为 key)
        const cached = getCachedJwt(this.authorization);
        if (cached) {
            this.jwtToken = cached.jwtToken;
            const remainMin = Math.round((cached.expireAt - Date.now()) / 60000);
            log(`命中 jwtToken 缓存, 剩余约 ${remainMin} 分钟`);
            const valid = await this.verifyJwt(true);
            if (valid) return true;
            log('缓存的 jwtToken 已失效, 重新换发 ...');
        }

        // 3. Authorization → ssoToken
        return await this.exchangeJwtFromAuth();
    }

    /** Authorization → ssoToken → jwtToken 实际换发 (含重试) */
    async exchangeJwtFromAuth(retryCount = 0) {
        log(`Authorization 换发 jwtToken${retryCount > 0 ? ` (第${retryCount + 1}次尝试)` : ''} ...`);

        // 3a. Authorization → ssoToken (orches.yun.139.com)
        if (!this.ssoToken) {
            this.ssoToken = await getSsoTokenByAuth(this.authorization, this.phone);
            if (!this.ssoToken) {
                log('ssoToken 换发失败, 请检查 Authorization 和手机号是否正确');
                if (retryCount < 1) {
                    log('等待 3 秒后重试 ...');
                    await sleep(3000);
                    return await this.exchangeJwtFromAuth(retryCount + 1);
                }
                return false;
            }
            log('ssoToken 换发成功');
        ...
        // 3b. ssoToken → jwtToken (优先用 caiyun 旧版接口, 失败回退到 ycloud 签名接口)
        let jwt = await getJwtByCaiyun(this.ssoToken);
        if (!jwt) {
            log('caiyun 接口换发失败, 尝试 ycloud 签名接口 ...');
            // 回退: 使用现有的 Api.tyrzLogin (m.mcloud.139.com/ycloud/auth-service/auth/tyrzLogin)
            try {
                const res = await Api.tyrzLogin(this.ssoToken, this.marketName);
                if (res.data && res.data.code === 0 && res.data.result && res.data.result.token) {
                    jwt = res.data.result.token;
                }
            } catch (e) {
                log(`ycloud tyrzLogin 异常: ${e.message}`);
            }
        }

        if (jwt) {
            this.jwtToken = jwt;
            const udId = extractUserDomainId(jwt);
            setCachedJwt(this.authorization, jwt, udId);
    ...
    /** jwtToken 失效 (90001) 时的自动恢复 */
    async refreshOnExpire() {
        if (this.type === 'auth') {
            log('检测到 jwtToken 失效, 使用 Authorization 重新换发 ...');
            // 清除缓存
            const key = hashToken(this.authorization);
            delete JWT_CACHE[key];
            this.ssoToken = '';  // 清除 ssoToken, 重新获取
            return await this.exchangeJwtFromAuth();
        }
        if (this.type === 'sso') {
...
async function main() {
    ...
    if (!tokens.length) {
        ...
        log('  说明 (支持三种格式, 自动识别):');
        log('  ① Authorization值#手机号  (推荐, 最稳定, 抓包 orches.yun.139.com)');
        log('  ② ssoToken  (YZsidssolg开头, 抓包 m.mcloud.139.com Referer 中 token=)');
        log('  ③ jwtToken  (eyJ开头, 2小时有效期)');
        log('  多账号用换行或 & 分隔');
        return;
    }
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        // 自动识别 token 类型 (若未显式指定或设为 auto)
        let detected = detectTokenType(tokens[i]);
        let type = (TOKEN_TYPE === 'auto') ? detected.type : TOKEN_TYPE;
        // auth 模式需要 authorization 和 phone
        const authorization = detected.type === 'auth' ? detected.authorization : '';
        const phone = detected.type === 'auth' ? detected.phone : PHONE;

        log(`Token 类型: ${type}${type === 'auth' ? ` (手机号: ${phone || '(空)'})` : ''}`);

        const account = new McloudAccount(
            type === 'auth' ? detected.authorization : detected.token,
            type, MARKET_NAME, authorization, phone
        );
        try {
