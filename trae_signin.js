 ...
 * # cron: 30 8 * * *
 *
 * Trae AI 每日自动签到脚本 v4（适配青龙面板 2.10+ / Node.js）
 ...
 *        · 网页版 F12 → Network → 找任意 api.trae.cn / api.trae.com.cn 请求
 *        · 复制请求头 Cookie 的完整值
 *
 * v4 更新：自动适配双域名（api.trae.cn / api.trae.com.cn）、输出凭证类型诊断
 */
...
/* ================= 可调配置 ================= */
const HOSTS          = ['api.trae.cn', 'api.trae.com.cn'];                   // 国服双域名，按序尝试
...
/* 用 Cookie 换取 JWT（复刻网页端 GetUserToken 接口逻辑，双域名自动尝试） */
async function exchangeToken(cookie) {
  ...
  for (const host of HOSTS) {
    ...
    if (r.status === 0) { lastErr = `${host}: ${r.error ? r.error.message : '网络异常'}`; continue; }
    const j = parseJson(r.body);
    const token = j && j.data && j.data.Result && j.data.Result.Token;
    if (token) return { ok: true, token: token, host: host };
    lastErr = `${host}: HTTP ${r.status} ${String(r.body).slice(0, 120)}`;
  }
...
async function signOne(raw, index, total) {
  try {
    ...
    if (!cred) {
      console.log(`[账号 ${index}/${total}] 【凭证无法识别】既不是 JWT 也不是 Cookie（首字符为 "${raw.trim().slice(0, 30)}"），已跳过`);
      return;
    }
    ...
    if (cred.type === 'jwt') {
      ...
      console.log(`[账号 ${index}/${total}] [诊断] 凭证类型 = Cookie，正在调用 GetUserToken 换取 JWT...`);
      ...
      token = ex.token;
      host = ex.host;
      console.log(`[账号 ${index}/${total}] [诊断] 换取成功（域名 ${host}）`);
    }
