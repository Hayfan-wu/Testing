...
### 20260728
v5.0.10:
- 签到 deviceId 优先从设备指纹接口获取，降低“活动太火爆/锁定失败”。

### 20260728
v5.0.9:
- 修复缓存 Token 每次运行都强制刷新的问题。
- expiresAt 缺失时优先复用已有 Token，并按默认有效期补齐。
...
2.登录https://sm.linzixuan.top 获取。(网站随时失效，不保证一直可用)

变量名: ydyp_device_id 或 YDYP_DEVICE_ID（脚本自动生成，可不指定）
...
TOKEN_VALID_TIME = 30 * 24 * 60 * 60 * 1000  # 默认缓存有效期 30 天
TOKEN_REFRESH_ADVANCE = 24 * 60 * 60 * 1000  # 到期前 24 小时再刷新
TOKEN_REFRESH_COOLDOWN = 6 * 60 * 60 * 1000  # 强制刷新失败后冷却 6 小时
...
def estimate_token_expires_at(token, account='', fallback_from=None):
    """从缓存/Authorization 估算有效期；无法解析时按默认 30 天。"""
    now = current_millis()
    base = int(fallback_from or now)
    raw = extract_raw_token(token, account)
    # 常见 raw token: xxx|1|RCS|<millis>|<suffix>
    for part in str(raw or '').split('|'):
        ...
        if value >= 10**12:
            ...
            if value > now:
                return value
            return value + TOKEN_VALID_TIME
        # 秒时间戳
        if value >= 10**9:
            millis = value * 1000
            if millis > now:
                return millis
            return millis + TOKEN_VALID_TIME
    return base + TOKEN_VALID_TIME
...
def ensure_account_storage_entry(account, token=''):
    ...
    if normalized_token and account_data.get('token') != normalized_token:
        ...
        # 新写入 token 时同步估算有效期，避免 expiresAt=0 导致每次强制刷新
        account_data['expiresAt'] = estimate_token_expires_at(
            normalized_token,
            account,
            account_data.get('lastRefreshAt') or current_millis(),
        )
        if not account_data.get('lastRefreshAt'):
            account_data['lastRefreshAt'] = current_millis()
        changed = True
    if 'expiresAt' not in account_data:
        account_data['expiresAt'] = 0
        changed = True
    if normalized_token and int(account_data.get('expiresAt') or 0) <= 0:
        account_data['expiresAt'] = estimate_token_expires_at(
            normalized_token,
            account,
            account_data.get('lastRefreshAt') or current_millis(),
...
def print_device_id_notice():
    print("deviceId 获取顺序: 设备指纹接口 → ydyp_device_id → 本地缓存 → 自动生成")
    if get_env_device_id():
        print("已检测到 ydyp_device_id，指纹接口失败时将使用环境变量 deviceId")
...
class YP:
    ...
    def load_or_create_market_device_profile(self):
        """加载 deviceId：优先设备指纹接口，其次环境变量/缓存，最后本地生成"""
        ensure_account_storage_entry(self.account, self.Authorization)

        # 与可用的移动云盘.py一致：优先走设备指纹接口，避免本地假 deviceId 触发签到风控
    ...
    def should_refresh_authorization(self, force=False):
        ...
        stored = self.get_storage_record()
        now = current_millis()
        token = stored.get('token') or self.Authorization
        if not token:
            return True, '本地无缓存Token'

        expires_at = int(stored.get('expiresAt') or 0)
        last_refresh_at = int(stored.get('lastRefreshAt') or 0)
        # 历史缓存 expiresAt=0 时，先按 token 内容/默认有效期补齐，优先复用已有 Token
        if expires_at <= 0:
            expires_at = estimate_token_expires_at(token, self.account, last_refresh_at or now)
            try:
                save_token_info(self.account, normalize_authorization(token), expires_at, last_refresh_at or now)
                self.save_authorization_record(
                    token=normalize_authorization(token),
        ...
        if now >= expires_at:
            # 冷却期内即使过期也不反复打刷新接口，先继续复用
            if last_refresh_at and now - last_refresh_at < TOKEN_REFRESH_COOLDOWN:
                remain = (TOKEN_REFRESH_COOLDOWN - (now - last_refresh_at)) / 3600000
                return False, f'刷新冷却中（剩余 {remain:.1f} 小时），继续使用缓存Token'
            return True, '缓存Token已过期'
        if expires_at - now <= TOKEN_REFRESH_ADVANCE:
            hours = (expires_at - now) / 3600000
            if last_refresh_at and now - last_refresh_at < TOKEN_REFRESH_COOLDOWN:
                return False, f'Token 将在 {hours:.1f} 小时后过期，但仍在刷新冷却期，继续使用缓存Token'
    ...
    def refresh_authorization_token(self, force=False):
        ...
        if not refresh_data:
            ...
            # 记录刷新时间，避免失败后每次运行都重复刷新
            self.save_authorization_record(token=self.Authorization, refreshed=True)
            return True
    ...
    def sync_token_storage(self):
        ...
        last_refresh_at = int(stored.get('lastRefreshAt') or 0)
        if expires_at <= 0:
            expires_at = estimate_token_expires_at(
                self.Authorization,
                self.account,
                last_refresh_at or current_millis(),
            )
    ...
    def build_tokenpk_page_url(self):
        return f'{self.market_base_url}/portal/yunClound/index.html?path={TOKENPK_MARKET_NAME}' f'&sourceid={TOKENPK_SOURCE_ID}&enableShare=1'
    ...
    def receive_pending_tokenpk_task_prizes(self):
        ...
        for task in tasks:
            if task.get('state') == 'SUCCESS' and task.get('taskType') != 'RESERVE':
                self.receive_tokenpk_task_prize(task)
            elif task.get('state') == 'WAIT' and self.build_tokenpk_task_payload(task).get('key') == 'shareFile':
    ...
    def assist_tokenpk_target(self):
        ...
        if stored.get('tokenpkAssistCode') == TOKENPK_INVITE_CODE and stored.get('tokenpkAssistMonth') == current_month:
    ...
    def request_red_packet_json(self, path_url, data=None, token=None, debug=None, retries=5):
        ...
        return self.request_json(url, headers=self.build_red_packet_headers(token if token is not None else self.red_packet_token), data=data or {}, method='POST', debug=debug, retries=retries)
    ...
    def login_red_packet(self):
        ...
        if not sso_token:
            ...
            return False
        login_data = self.request_red_packet_json(
            '/user/tokenValidate',
            data={
                'version': '1.0',
                'pintype': 13,
    ...
    def get_red_packet_account_info(self):
        return self.request_red_packet_json(
            '/redpacket/userAccountInfo',
            data={
    ...
    def user_sign_red_packet(self):
        data = self.request_red_packet_json(
            '/redpacket/userSign',
            data={
                'version': RED_PACKET_VERSION,
                'platformType': 1,
    ...
    def get_red_packet_cloud_phones(self):
        ...
        for status in ('ACTIVE', 'PENDING'):
            data = self.request_red_packet_json(
                '/thirdapi/huawei/thirdQueryUserRelationshipV2',
                data={
                    'pageNumber': 1,
                    'pageSize': 10,
                    'status': status,
    ...
    def get_red_packet_install_status(self, hw_token, instance_id, package_name):
        data = self.request_red_packet_json(
            '/app/activity/instanceApkInstallStatus',
            data={
                'hwToken': hw_token,
    ...
    def poll_red_packet_install_task(self, hw_token, task_id):
        for _ in range(60):
            data = self.request_red_packet_json(
                '/app/activity/appInstallTaskStatus',
                data={
                    'taskId': task_id,
                    'hwToken': hw_token,
                },
        ...
        for app in apps:
            ...
            if self.get_red_packet_install_status(hw_token, instance_id, package_name) == 2:
                ...
                if refreshed.get('userStatus') == 1:
                    ...
                    return True
                continue
            install_data = self.request_red_packet_json(
                '/app/activity/apkInstall',
                data={
    ...
    def get_ai_camera_sample_base64(self):
        ...
        image = signature + png_chunk(b'IHDR', header) + png_chunk(b'IDAT', zlib.compress(bytes(pixels))) + png_chunk(b'IEND', b'')
